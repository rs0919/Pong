...
{
    // Player two (top paddle) has same movement as bottom paddle,
    // but it's controlled through the A and D keys, instead of the arrow keys.
    ...
    void Update()
    {
        if (Input.GetKey(KeyCode.D) && (!touchingRightEdge))
        {
            playerPos.x += playerSpeed * Time.deltaTime;
        }

        if (Input.GetKey(KeyCode.A) && (!touchingLeftEdge))
        {
            playerPos.x -= playerSpeed * Time.deltaTime;
        }
